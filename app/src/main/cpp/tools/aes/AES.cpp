...
char *AES_ECB_PKCS7_Encrypt(const char *in, const uint8_t *key) {
    KEYLEN = strlen(reinterpret_cast<const char *const>(key));
    uint8_t *paddingInput = getPKCS7PaddingInput(in);
    int paddingInputLengt = strlen(reinterpret_cast<const char *const>(paddingInput));
    int count = paddingInputLengt / BLOCK_SIZE;
    //开始分段加密
    char *out = (char *) malloc(paddingInputLengt);
    for (int i = 0; i < count; ++i) {
        AES_ECB_encrypt(paddingInput + i * BLOCK_SIZE, key,
                        reinterpret_cast<uint8_t *>(out + i * BLOCK_SIZE));
    }
    char *base64En = b64_encode(reinterpret_cast<const unsigned char *>(out), paddingInputLengt);
    free(paddingInput);
    free(out);
    return base64En;
}
...
char *AES_ECB_PKCS7_Decrypt(const char *in, const uint8_t *key) {
    ...
    uint8_t *out = static_cast<uint8_t *>(malloc(inputLength));
...
char *AES_CBC_PKCS7_Encrypt(const char *in, const uint8_t *key, const uint8_t *iv) {
    ...
    int paddingInputLengt = strlen(reinterpret_cast<const char *const>(paddingInput));
    char *out = (char *) malloc(paddingInputLengt);
    AES_CBC_encrypt(reinterpret_cast<uint8_t *>(out), paddingInput, paddingInputLengt, key, iv);
    char *base64En = b64_encode(reinterpret_cast<const unsigned char *>(out), paddingInputLengt);
    free(paddingInput);
    free(out);
    return base64En;
...
char *AES_CBC_ZREO_Encrypt(const char *in, const uint8_t *key, const uint8_t *iv) {
    ...
    AES_CBC_encrypt(reinterpret_cast<uint8_t *>(out), paddingInput, size, key, iv);
    char *base64En = b64_encode(reinterpret_cast<const unsigned char *>(out), size);
    free(paddingInput);
    free(out);
    return base64En;
}
